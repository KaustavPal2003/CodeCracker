...
class CompareManager {
    ...
    cacheElements() {
        const ids = [
            ...
            'notification', 'error-message', 'user1-error', 'user2-error' // Issue 11
        ];
        const elements = {};
        for (const id of ids) {
    ...
    initialize() {
        ...
        if (this.user1Username) {
            initWebSocket(this.user1Username, 'compare', { updateStats, updateHistoryTable, updateChart });
    ...
    checkInitialUserStatus() {
        checkUserStatus(this.user1Username)
            .then(data => {
                ...
                this.elements['user1-error'].textContent = data.exists_in_sqlite ? '' : 'Invalid username'; // Issue 11
            ...
            .catch(error => {
                ...
                this.elements['user1-error'].textContent = 'Error validating username'; // Issue 11
    ...
    async updateCompareButton() {
        ...
        if (!user1 || !user2 || user1 === user2 || !this.isValidUsername(user1) || !this.isValidUsername(user2)) {
            ...
            if (user1 === user2) {
                ...
                this.elements['user2-error'].textContent = 'Cannot compare the same username'; // Issue 11
                this.elements['user2-error'].style.display = 'block';
            } else if (!this.isValidUsername(user1)) {
                this.elements['user1-error'].textContent = 'Invalid username format'; // Issue 11
                this.elements['user1-error'].style.display = 'block';
            } else if (!this.isValidUsername(user2)) {
                this.elements['user2-error'].textContent = 'Invalid username format'; // Issue 11
                this.elements['user2-error'].style.display = 'block';
            }
            return;
        }
        ...
        try {
            ...
            if (!user1Data.exists_in_sqlite) {
                this.elements['user1-error'].textContent = 'User not found'; // Issue 11
                this.elements['user1-error'].style.display = 'block';
            }
            if (!user2Data.exists_in_sqlite) {
                this.elements['user2-error'].textContent = 'User not found'; // Issue 11
                this.elements['user2-error'].style.display = 'block';
            }
            if (!user1Data.exists_in_mongodb || !user1Data.has_valid_ratings) {
                this.elements['user1-error'].textContent = 'No ratings or history for this user'; // Issue 11
                this.elements['user1-error'].style.display = 'block';
            }
            if (!user2Data.exists_in_mongodb || !user2Data.has_valid_ratings) {
                this.elements['user2-error'].textContent = 'No ratings or history for this user'; // Issue 11
                this.elements['user2-error'].style.display = 'block';
            }
        } catch {
            this.elements['compare-button'].disabled = true;
            ...
            this.elements['user1-error'].textContent = 'Error validating username'; // Issue 11
            this.elements['user2-error'].textContent = 'Error validating username'; // Issue 11
            this.elements['user1-error'].style.display = 'block';
            this.elements['user2-error'].style.display = 'block';
        }
    }
    ...
    async handleSubmit(event) {
        ...
        try {
            ...
            this.elements['user1-error'].textContent = `Comparison failed: ${error.message}`; // Issue 11
            this.elements['user2-error'].textContent = `Comparison failed: ${error.message}`; // Issue 11
            this.elements['user1-error'].style.display = 'block';
            this.elements['user2-error'].style.display = 'block';
        } finally {
            this.isComparing = false;
