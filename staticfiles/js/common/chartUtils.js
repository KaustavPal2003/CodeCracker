// codecracker/static/js/common/chartUtils.js
...
function getChartColors(mode = 'performance') {
    ...
    // Issue 6: Colorblind-friendly colors
    const platforms = {
        codeforces: { main: isDarkTheme ? '#1e90ff' : '#4169e1', compare: '#ff4500' }, // Blue and orange-red
        codechef: { main: isDarkTheme ? '#ff6347' : '#dc143c', compare: '#8a2be2' }, // Red and purple
        leetcode: { main: '#32cd32', compare: '#ffa500' } // Green and orange
    };
...
function getChartElements() {
    return {
        ...
        detailsDiv: document.getElementById('chart-details') // Issue 7
    };
}
...
export function updateChart(user1Data, compareData, user1Username, user2Username, mode = 'performance') {
    ...
    if (!elements.ctx || !elements.noDataMessage || !elements.zoomToFit || !elements.resetZoom || !elements.detailsDiv) {
        console.error('Missing required chart DOM elements');
        showError('Failed to render chart: Missing canvas, controls, or details container');
        return;
    }
    ...
    const datasets = mode === 'compare' ? platforms.flatMap(platform => [
        {
    ...
    window.chart = new Chart(elements.ctx, {
        ...
        options: {
            ...
            onClick: (event, elements) => { // Issue 7: Accessible click handler
                if (elements.length) {
