...
import { showError, showNotification } from './common/utils.js';
import { animateStats } from './compare/uiUtils.js'; // Assuming uiUtils.js is in compare directory
...
export function initWebSocket(username, mode = 'performance', { updateStats, updateHistoryTable, updateChart } = {}) {
    ...
    console.log(`WebSocket initialized for ${username} in ${mode} mode`);
    ...
    ws.onmessage = function (event) {
        try {
            ...
            // Update UI for comparison data if in compare mode
            ...
            // Update chart and history table if functions are provided
            if (updateChart) {
    ...
    ws.onclose = function (event) {
        ...
        if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS && event.code !== 1000) {
            ...
            setTimeout(() => {
                reconnectAttempts++;
                initWebSocket(username, mode, { updateStats, updateHistoryTable, updateChart });
            }, delay);
        } else {
...
export function sendWebSocketMessage(message, mode = 'performance') {
    return new Promise((resolve, reject) => {
        if (!ws || ws.readyState !== WebSocket.OPEN) {
            ...
            if (mode === 'compare') {
                pendingComparison = message; // Store for retry only in compare mode
            }
            reject(error);
            return;
        }
